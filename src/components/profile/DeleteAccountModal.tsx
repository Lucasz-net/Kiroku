...
export const DeleteAccountModal = ({ username, onClose }: DeleteAccountModalProps) => {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md bg-[var(--kr-surface)] border border-[#FF3B3B]/30 rounded-2xl shadow-[0_24px_80px_rgba(0,0,0,0.9)] overflow-hidden">
        ...
        <div className="p-8">
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-black text-[var(--kr-text)]">Eliminar cuenta</h2>
            </div>
            <button
              onClick={onClose}
              type="button"
          ...
          <label className="block text-[11px] font-bold text-zinc-500 uppercase tracking-widest mb-2">
            Escribí <span className="text-[var(--kr-text)] normal-case">{username}</span> para confirmar
          </label>
          <input
            type="text"
            value={confirmText}
            ...
            autoFocus
            className="w-full bg-[var(--kr-surface-sunken)] border border-[#FF3B3B]/15 focus:border-[#FF3B3B]/50 focus:ring-1 focus:ring-[#FF3B3B]/20 text-[var(--kr-text)] rounded-xl px-4 py-3 text-sm outline-none transition-all mb-4"
          />
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              type="button"
              className="flex-1 py-3 bg-[var(--kr-surface-sunken)] border border-[#FF3B3B]/15 text-zinc-400 hover:text-[var(--kr-text)] font-black text-xs uppercase tracking-widest rounded-xl transition-colors"
            >
              Cancelar
            </button>
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#FF3B3B] hover:bg-[#e02d2d] disabled:opacity-30 disabled:cursor-not-allowed text-[var(--kr-text)] font-black text-xs uppercase tracking-widest rounded-xl transition-colors"
            >
