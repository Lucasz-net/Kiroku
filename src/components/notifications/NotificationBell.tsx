  ...
  return (
    <div className="relative" ref={rootRef}>
      <button
        ...
        aria-expanded={open}
        className={`relative flex items-center justify-center w-9 h-9 md:w-10 md:h-10 bg-[#11131A]/80 border rounded-lg transition-colors ${
          open
      ...
      >
        <Bell size={17} />
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[17px] h-[17px] px-1 flex items-center justify-center rounded-full bg-[#FF3B3B] text-white text-[9px] font-black tabular-nums border-2 border-[#11131A]">
            {unread > 99 ? '99+' : unread}
          </span>
        )}
      </button>
      ...
      {open && (
        ...
        <div className="fixed inset-x-4 top-[84px] z-50 md:absolute md:inset-x-auto md:top-full md:right-0 md:mt-3 md:w-[22rem] bg-[#11131A] border border-[#FF3B3B]/25 rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.7)] overflow-hidden">
          ...
          <Link
            ...
            className="block px-4 py-3 border-t border-[#FF3B3B]/10 text-center text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-[#FF3B3B] hover:bg-white/[0.02] transition-colors"
          >
